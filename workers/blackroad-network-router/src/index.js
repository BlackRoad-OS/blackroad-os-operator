 ...
 * Routes blackroad.network traffic to the BlackRoad Network landing page
 * @owner Alexa Louise Amundson
 * @system BlackRoad OS
 */

const PAGES_ORIGIN = 'https://blackroad-os-web.pages.dev';
const CONTENT_PATH = '/blackroad-network/';
  ...
  async fetch(request, env) {
    ...
    if (path === '/api' || path === '/api/') {
      return new Response(JSON.stringify({
        ...
        headers: { 'Content-Type': 'application/json', 'X-Served-By': 'blackroad-network-router' }
      });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Service unavailable', domain: 'blackroad.network' }), {
